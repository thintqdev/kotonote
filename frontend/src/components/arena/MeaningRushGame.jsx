...
export default function MeaningRushGame({ game, onComplete }) {
	...
	return (
		<div className="arena-game arena-game--meaning">
			<div className="arena-game-head">
				<h2 className="arena-game-title">
					{game.titleVi || t('arenaPage.game.meaning_rush')}
				</h2>
			...
			<p className="arena-game-hint">{t('arenaPage.meaningHint')}</p>
			{current ? (
				<div className="arena-reading-card">
					<p className="arena-reading-word">{current.wordJa}</p>
					{current.reading ? (
						<p className="arena-meaning-reading">{current.reading}</p>
					) : null}
					<div className="arena-particle-choices arena-meaning-choices" role="group">
